...
В проекте есть две версии модели: `model_v1.onnx` и `model_v2.onnx`. Их можно выбрать явно через model_version или автоматически через client_id. По сути это одна и та же модель, но разделена в два разных логических объекта, для интерфейса AB теста
